...
// local
$user = "dbUser"; //Enter the user name
$password = "9JGxnxmxCo454ZEF"; //Enter the password
$host = "127.0.0.1"; //Enter the host
$dbase = "testdata"; //Enter the database
...
//Loading Team List
if ($queryString == 'getList') {

	//Define values to pass from HTML on to SQL
	// $usernameView='demo' ;  //Pass username to identify associated Team
...
} else if (isset($output['saveTeam']) && $output['saveTeam']==1) {
		
	//Define values to pass from HTML on to SQL
	$newTeamName= $output['teamName'];
	$createdBy= 'demo';
		
	$teamSQL = "INSERT INTO teamInfo (`teamName`, `createdBy`)
	VALUES ('$newTeamName','$createdBy')";
...
} else if (isset($output['teamViewData'])){
	
	//team ID details
	$teamID = $output['teamViewData'];
...
} else if (isset($output['saveGameSheet'])){
...
if($sheetID == 0){
	...
	if (!$resultNewSheet)
	{
		...
		// var_dump($updateRowsToPositionInfoSQL);
	...
	function loadGameSheetRow($teamID) {
...
} else if (isset($output['playerRecord'])){
	...
	//teamID is missing... add to query;compare with dB Table
	...
	if (!$resultPlayerRecord) {
			...
			// echo $playerRecordAffected . "line 320 ";
...
} else if (isset($output['deletePlayerRow'])){
...
} else if (isset($output['loadGameSheet'])){
	
	$gameSheetToLoad =$output['loadGameSheet'];
	
	// var_dump($gameSheetToLoad);
	...
	if (!$gameSheetPositionsSQLResult)
	{
		...
		for ($gameSheetPackage['positions']=array(); 
		$rowfP = mysqli_fetch_assoc($gameSheetPositionsSQLResult); 
		$gameSheetPackage['positions'][] = $rowfP);
